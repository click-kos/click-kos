...
function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    const checkPaymentStatus = async () => {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/payments/${paymentId}`);
        if (response.ok) {
          const data = await response.json();
          const payment = (data && (data.payment ?? data)) as { status?: 'pending' | 'success' | 'failed' } | undefined;
        ...
        if (sessionId) {
          const confirm = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/payments/confirm`, {
            method: 'POST',
